function vumeter(elem, config){
    ...
    var boxCountRed     = config.boxCountRed    || 5   ;
    var boxCountYellow  = config.boxCountYellow || 10  ;
    var boxGapFraction  = config.boxGapFraction || 0.20;
    var jitter          = config.jitter         || 0.02;
    // vumeter(gain_1, { "boxCount": 40, "boxGapFraction": 0.1, "max": 100, "boxCountRed":5, "boxCountYellow":10}
    // Colours
...
function calculateRMS(audioBuffer) {
  var sumOfSquares = 0;
  var samples = audioBuffer.getChannelData(0); // Assuming mono audio

  for (var i = 0; i < samples.length; i++) {
    sumOfSquares += samples[i] * samples[i];
  }

  rms = Math.sqrt(sumOfSquares / samples.length) * 400 ;
  rms = parseInt(rms);
  console.log('rms : ' + rms);
  gain_1.setAttribute('data-val', rms);
  return rms;
}
