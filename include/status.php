...
<fieldset style="background-color:#e8e8e8e8;width:160px;margin-top:6px;;margin-bottom:0px;margin-left:0px;margin-right:3px;font-size:12px;border-top-left-radius: 10px; border-top-right-radius: 10px;border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
  <?php
    $muInfo      = read_dvsmu();
    $user        = $muInfo['CUR_USER'];    
    $ABINFO      = $muInfo[$user]['rxPort'];
    $mb_ini_path = $muInfo[$user]['MMDVMINIPATH'].'/'.MMDVMINIFILENAME;
    $mmdvmconfgs = getMMDVMConfig($mb_ini_path);
    if ($user=='MAIN') {$user_no='';}
    else               {$user_no=substr($user, -2);}

    $testMMDVModeDMR = getConfigItem("DMR", "Enable", $mmdvmconfigs);
    if ( $testMMDVModeDMR == 1 or true) { //Hide the DMR information when DMR mode not enabled.
      ...
      // echo ABINFO;
      $file_ABInfo = '/tmp/ABInfo_'.$ABINFO.'.json';
      if (file_exists($file_ABInfo)) {
          $abinfo = getABInfo($file_ABInfo);
      ...
      if (getEnabled("DMR Network", $mmdvmconfigs) == 1) {
      	if ($dmrMasterHost == '127.0.0.1' && isProcessRunning("DMRGateway")) {
            ...
            if ( !isset($configdmrgateway['XLX Network 1']['Enabled']) && isset($configdmrgateway['XLX Network']['Enabled']) && $configdmrgateway['XLX Network']['Enabled'] == 1) {
          		if (file_exists("/var/log/mmdvm/DMRGateway-".gmdate("Y-m-d").".log")) {
                $xlxMasterHost1 = exec('grep -a \'XLX, Linking\|Unlinking\' /var/log/mmdvm/DMRGateway-'.gmdate("Y-m-d").'.log | tail -1 | awk \'{print $5 " " $8 " " $9}\'');
          		} else {
                $xlxMasterHost1 = exec('grep -a \'XLX, Linking\|Unlinking\' /var/log/mmdvm/DMRGateway-'.gmdate("Y-m-d", time() - 86340).'.log | tail -1 | awk \'{print $5 " " $8 " " $9}\'');
              }
          ...
          $file_mb_log = "/var/log/mmdvm/MMDVM_Bridge".$user_no."-".gmdate("Y-m-d").".log";
      		if (file_exists($file_mb_log)) {
            $dmrstat = exec('grep -a \'DMR, Logged\|DMR, Closing DMR\|DMR, Opening DMR\|DMR, Connection\' /var/log/mmdvm/MMDVM_Bridge'.$user_no.'-'.gmdate("Y-m-d").'.log | tail -1 | awk \'{print $5 " " $10}\'');
      		} else {
            $dmrstat = exec('grep -a \'DMR, Logged\|DMR, Closing DMR\|DMR, Opening DMR\|DMR, Connection\' /var/log/mmdvm/MMDVM_Bridge'.$user_no.'-'.gmdate("Y-m-d", time() - 86340).'.log | tail -1 | awk \'{print $5 " " $10}\'');
          }
          ...
          }	elseif (strpos($dmrstat, 'Opening') !== false || strpos($dmrstatus, 'Closing') !== false || strpos($dmrstatus, 'Connection') !== false) {
