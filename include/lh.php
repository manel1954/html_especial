...
-->
<fieldset style="box-shadow:0 0 10px #999;background-color:#e8e8e8e8; width:640px;margin-top:0px;margin-left:0px;margin-right:0px;
  ...
  <?php
    $last_call = $lastHeard[0][2];
    $last_name = $lastHeard[0][3];
    $last_time = $lastHeard[0][0]; $tm = time()-3600*9-strtotime(date($last_time));
    if ($tm<20 or true) {qrz_image_url($last_call); $rw=14;} else {$rw=19;}
  ?>
...
<?php
  ...
  for ($i = 0;  ($i <= $rw); $i++) { //Last 20 calls
      ...
      $md = str_replace('Slot 2', 'S2', $listElem[1]); $md = str_replace('Slot 2', 'S2', $md);
  		echo"<td align=\"left\" style=\"color:green; font-weight:bold;\">&nbsp;$md</td>";
