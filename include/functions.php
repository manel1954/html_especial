...
function getMMDVMConfig($mb_ini_path='') {
  if ($mb_ini_path=='') {
    $mb_ini_path=MMDVMINIPATH."/".MMDVMINIFILENAME;}
	// loads MMDVM_Bridge.ini into array for further use
	$conf = array();
	if ($configs = @fopen($mb_ini_path, 'r')) {
...
function webproxy_activate($USER_list) {
  ...
  if (strlen($all_webproxy)>0) {
    ...
    echo '<br>', $enable_webproxy;
    echo '<br>', $restart_web_AB, '<br>';
    exec($enable_webproxy);
    exec($restart_web_AB );
  }
}

function qrz_image_url($call) {
  if ($call=='') {
    $image_qrz = $image_flg = './flag/dvs.png';
    $dmrid = $name1 = $addr1 = 'DVSwitch';
    $tz    = date("H:i:s ").'LocalTime';
  } elseif ($call=='4000') {
    $image_qrz = $image_flg = './flag/ulk.png';
    $dmrid = $name1 = $addr1 = 'UnLink';
    $tz    = date("H:i:s ").'LocalTime';
  } elseif ($call=='450997') {
    $image_qrz = $image_flg = './images/prt.png';
    $dmrid = $name1 = $addr1 = 'DMR+ IPSC2-Korea';
    $tz    = date("H:i:s ").'LocalTime';
  } elseif ($call=='4750') {
    $image_qrz = $image_flg = './flag/dmrplus.png';
    $dmrid = $name1 = $addr1 = 'Echo Test (PARROT)';
    $tz    = date("H:i:s ").'LocalTime';
  } else {
    $image_qrz = '/var/www/html/qrz_photo/'.$call.'.png';
    $image_flg = './flag/'.call2flag($call);
    if (file_exists($image_qrz)) {
      ...
      if (strlen($image_url)<50) {
        ...
        file_put_contents($image_qrz, file_get_contents($image_url));
        $image_qrz = './qrz_photo/'.$call.'.png';
      }
    }
    // 이름, 주소
    ...
    if (substr_count($data[0], ',')==6) {
      ...
      list($dmrid, $callsign, $name1, $name2, $addr1, $addr2, $addr3) = array('no dmrid', $call, 'no name info in dmrid database', '', call2flag($call, FALSE), '', '');
    }
  ...
  echo '
    ...
    <table style="margin-top:3px;">
      <tr><td rowspan=3 width=140px>'.'<img src='.$image_qrz.' width=115px height=100px></td>
          <td rowspan=3 width=140px>'.'<img src='.$image_flg.' width=115px height=100px></td>
          <th style=font-size:16pt id=call><b>'.$call .'</th>
          <th style=font-size:12pt><b>'.$dmrid.'</th>
          <th style=font-size:12pt><b>'.$tz.'</th>
      </tr>
      <tr><td colspan=3 style=font-size:14pt><b>'.$name1.' '.$name2.'</td></tr>
      <tr><td colspan=3 style=font-size:12pt width=100%>'.$addr1.' '.$addr2.'<br>'.$addr3.'</td></tr>
    </table>';
}
...
function down_user_css() {
  ...
  if (!file_exists($user_csv)) {
    ...
    if ((time()-$mtime)>7200 or $filesize < 13000000) {exec($cmd_down);}
  }
}
